...
DWORD WINAPI train(LPVOID lpParam) {
    ...
    while (1) {
        ...
        HFONT font = CreateFont(100, 0, 0, 0, FW_BOLD, 0, 0, 0, 0, 0, 0, 0, 0, TEXT("Arial"));
        SelectObject(hdc, font);
        SetBkMode(hdc, TRANSPARENT);
        SetTextColor(hdc, RGB(255, 255, 255));
        TextOutA(hdc, (int)(w/2 + cos(angle)*150), (int)(h/2 + sin(angle)*100), "SaltWin 1.5.1", 13);
        DeleteObject(font);
        angle += 0.1;
        ReleaseDC(0, hdc);
        Sleep(10);
...
DWORD WINAPI train2(LPVOID lpParam) {
    ...
    while (1) {
        ...
        SetBkMode(hdc, TRANSPARENT);
        SetTextColor(hdc, RGB(255, 255, 255));
        TextOutA(hdc, (int)(w/2 + cos(angle)*250), (int)(h/2 + sin(angle)*150), "SaltWin 1.5.1", 13);
        DeleteObject(font);
        angle += 0.05;
        ReleaseDC(0, hdc);
        Sleep(10);

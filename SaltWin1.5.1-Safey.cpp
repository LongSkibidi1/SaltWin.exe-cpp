...
DWORD WINAPI SaltWinMoving(LPVOID lpParam) {
    ...
    while (true) {
        ...
        HFONT hFont = CreateFont(100, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, 
        ...
        TextOutA(hdc, x, y, "SaltWin 1.5.1-Safey.exe", 13);
        
        SelectObject(hdc, hOldFont);
        DeleteObject(hFont);

        x += dx; y += dy;
        if (x <= 0 || x >= sw - 100) dx = -dx;
        if (y <= 0 || y >= sh - 50) dy = -dy;
        ReleaseDC(NULL, hdc);
        Sleep(20);
...
DWORD WINAPI balls(LPVOID lpParam) {
    ...
    while (1) {
        ...
        x += 10 * signX;
        y += 10 * signY;
        if (y >= GetSystemMetrics(SM_CYSCREEN) - 100 || y <= 0) signY = -signY;
        if (x >= GetSystemMetrics(SM_CXSCREEN) - 100 || x <= 0) signX = -signX;
        Sleep(10);
        DeleteObject(brush);
        ReleaseDC(0, hdc);
    }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    if (MessageBoxW(NULL, L"Run GDI only?", L"SaltWin.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) ExitProcess(0);
    if (MessageBoxW(NULL, L"Are u sure?", L"SaltWin.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) ExitProcess(0);

    CreateThread(NULL, 0, MouseErrorIcon, NULL, 0, NULL);
    Sleep(1000);

    HANDLE P1 = CreateThread(NULL, 0, payload1, NULL, 0, NULL);
    sound1();
    TerminateThread(P1, 0); CloseHandle(P1);
    Sleep(100);

    HANDLE P3 = CreateThread(NULL, 0, payload3, NULL, 0, NULL);
    sound3();
    TerminateThread(P3, 0); CloseHandle(P3);
    Sleep(100);

    HANDLE P4 = CreateThread(NULL, 0, Icons, NULL, 0, NULL);
    HANDLE P5 = CreateThread(NULL, 0, train, NULL, 0, NULL);
    HANDLE P_Salt = CreateThread(NULL, 0, SaltWinMoving, NULL, 0, NULL);
    sound4();
    TerminateThread(P4, 0); CloseHandle(P4);
    TerminateThread(P5, 0); CloseHandle(P5);
    TerminateThread(P_Salt, 0); CloseHandle(P_Salt);
    Sleep(100);

    HANDLE P6 = CreateThread(NULL, 0, train2, NULL, 0, NULL);
    sound5();
    TerminateThread(P6, 0); CloseHandle(P6);
    Sleep(100);
    ...
    sound6();
    TerminateThread(P8, 0); CloseHandle(P8);
    TerminateThread(P9, 0); CloseHandle(P9);
    TerminateThread(P10, 0); CloseHandle(P10);
